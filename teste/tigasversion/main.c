...
/* Atualiza IA e animação do pedinte */
static void updatePedinte(Pedinte* p, SDL_Rect player, Uint32 agora) {
    float distancia = fabs((player.x + player.w/2) - (p->pos.x + p->pos.w/2));

    switch (p->estado) {
        case INIMIGO_PARADO:
            if (distancia < p->distanciaVisao)
                p->estado = INIMIGO_LEVANTANDO;
            break;

        case INIMIGO_LEVANTANDO:
            // transição automática após animação
            break;
    ...
    if (p->estado == INIMIGO_LEVANTANDO) {
        ...
        p->dir = (player.x < p->pos.x) ? ESQUERDA : DIREITA;
...
void runGame(SDL_Window* win, SDL_Renderer* ren) {
	...
	// adiciona um pedinte no cenário inicial (ponte)
	initPedinte(&pedintes[numPedintes++], 3*w/5, (h - ((15*h)/100)/3) - 100 + 5, 110, 100);
    ...
    while (!SDL_QuitRequested()) {
	    ...
	    updatePedinte(&pedintes[i], player, agora);
	}
        ...
        // NPCs
        for (int i = 0; i < numPedintes; i++) {
	    renderPedinte(ren, texPedinte, &pedintes[i], camera);
	}
